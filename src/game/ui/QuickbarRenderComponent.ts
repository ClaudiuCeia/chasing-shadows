...
export class QuickbarRenderComponent extends HudRenderComponent {
  ...
  public override doRender(
    ...
    ctx.strokeStyle = "rgba(228, 214, 189, 0.7)";
    ctx.lineWidth = 2;
    ctx.strokeRect(frame.x, frame.y, frame.width, frame.height);

    const innerWidth = frame.width - 28;
    ...
    for (let i = 0; i < SLOT_COUNT; i++) {
      ...
      ctx.strokeStyle = "rgba(244, 233, 207, 0.68)";
      ctx.lineWidth = 1.5;
      ...
      if (this.itemSheet) {
        ...
        const iconSize = Math.floor(slotSize * 0.62);
        const iconX = Math.floor(x + (slotSize - iconSize) / 2);
        const iconY = Math.floor(y + (slotSize - iconSize) / 2) + 3;
